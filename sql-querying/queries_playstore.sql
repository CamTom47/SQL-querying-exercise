...
-- 5. Find the app that has the most reviews with a rating greater than equal to 4.8.
    -- playstore=# SELECT app_name FROM analytics WHERE reviews = (SELECT MAX(reviews) from analytics);                                                                                                                                                                                                                            app_name 
...
-- 8. Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.
    -- playstore=# SELECT app_name, rating, reviews, min_installs FROM analytics WHERE min_installs <= 50 AND reviews > 0 AND rating > 0 ORDER BY rating DESC;

-- 9. Find the names of all apps that are rated less than 3 with at least 10000 reviews.
    -- playstore=# SELECT app_name FROM analytics WHERE rating < 3 and reviews >= 10000;

-- 10. Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
    -- playstore=# SELECT * FROM analytics WHERE price BETWEEN .10 AND 1.00 ORDER BY reviews DESC LIMIT 10;
...
-- 12. Find the most expensive app (the query is very similar to #11).
    -- playstore=# SELECT * FROM analytics WHERE price = (SELECT MAX(price) FROM analytics);

-- 13. Count all the reviews in the Google Play Store.
    --playstore=# SELECT SUM(reviews) AS Total_Reviews FROM analytics;
